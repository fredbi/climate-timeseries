# coding: utf-8

"""
    Climate time series API

    The API allows the public to search and consult time series about climate change research conducted by TheShiftProject.  The climate time series API allows contributors to upload time series about their climate change models and studies.  Other secured endpoints allows admins to maintain the nomenclatures used by the climate time series, such as units etc. Timeseries publication status and ownership is for now managed by admins.   # noqa: E501

    OpenAPI spec version: v0.0.1
    Contact: fredbi@yahoo.com
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import re  # noqa: F401

# python 2 and python 3 compatibility library
import six

from swagger_client.api_client import ApiClient


class SeriesApi(object):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    Ref: https://github.com/swagger-api/swagger-codegen
    """

    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client

    def series_get(self, **kwargs):  # noqa: E501
        """Get all available series, with search filters  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_get(async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str glob: Filters the result according to a glob pattern applied on the short name of the requested object. 
        :param str tag: Filter the result by search for a given keyword, whenever tag search is applicable.  This parameter has no effect on objects where tag search is not applicable. 
        :param list[str] tags: Filter the result by search for one tag in the given list of keywords, whenever tag search is applicable.  Up to 5 tags can be searched.  This parameter has no effect on objects where tag search is not applicable. 
        :param str theme: A theme path (url-encoded) to be searched for. 
        :param str zone: A zone short code (url-encoded) to be searched for. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :param str owner_id: The ID of the contributor. 
        :param str datasource: A datasource short code to be searched for. 
        :param str email: Refers to some data owner by email. 
        :return: list[Series]
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_get_with_http_info(**kwargs)  # noqa: E501
        else:
            (data) = self.series_get_with_http_info(**kwargs)  # noqa: E501
            return data

    def series_get_with_http_info(self, **kwargs):  # noqa: E501
        """Get all available series, with search filters  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_get_with_http_info(async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str glob: Filters the result according to a glob pattern applied on the short name of the requested object. 
        :param str tag: Filter the result by search for a given keyword, whenever tag search is applicable.  This parameter has no effect on objects where tag search is not applicable. 
        :param list[str] tags: Filter the result by search for one tag in the given list of keywords, whenever tag search is applicable.  Up to 5 tags can be searched.  This parameter has no effect on objects where tag search is not applicable. 
        :param str theme: A theme path (url-encoded) to be searched for. 
        :param str zone: A zone short code (url-encoded) to be searched for. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :param str owner_id: The ID of the contributor. 
        :param str datasource: A datasource short code to be searched for. 
        :param str email: Refers to some data owner by email. 
        :return: list[Series]
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['glob', 'tag', 'tags', 'theme', 'zone', 'status', 'owner_id', 'datasource', 'email']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_get" % key
                )
            params[key] = val
        del params['kwargs']

        if self.api_client.client_side_validation and ('glob' in params and
                                                       len(params['glob']) > 100):
            raise ValueError("Invalid value for parameter `glob` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('glob' in params and
                                                       len(params['glob']) < 1):
            raise ValueError("Invalid value for parameter `glob` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        if self.api_client.client_side_validation and ('tag' in params and
                                                       len(params['tag']) > 100):
            raise ValueError("Invalid value for parameter `tag` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('tag' in params and
                                                       len(params['tag']) < 1):
            raise ValueError("Invalid value for parameter `tag` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        if self.api_client.client_side_validation and ('tags' in params and
                                            len(params['tags']) > 5):
            raise ValueError("Invalid value for parameter `tags` when calling `series_get`, number of items must be less than or equal to `5`")  # noqa: E501
        if self.api_client.client_side_validation and ('theme' in params and
                                                       len(params['theme']) > 100):
            raise ValueError("Invalid value for parameter `theme` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('theme' in params and
                                                       len(params['theme']) < 1):
            raise ValueError("Invalid value for parameter `theme` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        if self.api_client.client_side_validation and ('zone' in params and
                                                       len(params['zone']) > 100):
            raise ValueError("Invalid value for parameter `zone` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('zone' in params and
                                                       len(params['zone']) < 1):
            raise ValueError("Invalid value for parameter `zone` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        if self.api_client.client_side_validation and ('datasource' in params and
                                                       len(params['datasource']) > 100):
            raise ValueError("Invalid value for parameter `datasource` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('datasource' in params and
                                                       len(params['datasource']) < 1):
            raise ValueError("Invalid value for parameter `datasource` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        if self.api_client.client_side_validation and ('email' in params and
                                                       len(params['email']) > 100):
            raise ValueError("Invalid value for parameter `email` when calling `series_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('email' in params and
                                                       len(params['email']) < 1):
            raise ValueError("Invalid value for parameter `email` when calling `series_get`, length must be greater than or equal to `1`")  # noqa: E501
        collection_formats = {}

        path_params = {}

        query_params = []
        if 'glob' in params:
            query_params.append(('glob', params['glob']))  # noqa: E501
        if 'tag' in params:
            query_params.append(('tag', params['tag']))  # noqa: E501
        if 'tags' in params:
            query_params.append(('tags', params['tags']))  # noqa: E501
            collection_formats['tags'] = 'csv'  # noqa: E501
        if 'theme' in params:
            query_params.append(('theme', params['theme']))  # noqa: E501
        if 'zone' in params:
            query_params.append(('zone', params['zone']))  # noqa: E501
        if 'status' in params:
            query_params.append(('status', params['status']))  # noqa: E501
        if 'owner_id' in params:
            query_params.append(('ownerId', params['owner_id']))  # noqa: E501
        if 'datasource' in params:
            query_params.append(('datasource', params['datasource']))  # noqa: E501
        if 'email' in params:
            query_params.append(('email', params['email']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='list[Series]',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_post(self, series, **kwargs):  # noqa: E501
        """Creates a new time series  # noqa: E501

        This creates a new time time series.  If some values are specified, the initial version for the new series is created with these values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_post(series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_post_with_http_info(series, **kwargs)  # noqa: E501
        else:
            (data) = self.series_post_with_http_info(series, **kwargs)  # noqa: E501
            return data

    def series_post_with_http_info(self, series, **kwargs):  # noqa: E501
        """Creates a new time series  # noqa: E501

        This creates a new time time series.  If some values are specified, the initial version for the new series is created with these values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_post_with_http_info(series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series' is set
        if self.api_client.client_side_validation and ('series' not in params or
                                                       params['series'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series` when calling `series_post`")  # noqa: E501

        collection_formats = {}

        path_params = {}

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'series' in params:
            body_params = params['series']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_delete(self, series_id, **kwargs):  # noqa: E501
        """Deletes a time series  # noqa: E501

        All versions and values associated to this series are deleted.  Only the original owner of a series may delete it.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_delete(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_delete_with_http_info(series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_delete_with_http_info(series_id, **kwargs)  # noqa: E501
            return data

    def series_series_id_delete_with_http_info(self, series_id, **kwargs):  # noqa: E501
        """Deletes a time series  # noqa: E501

        All versions and values associated to this series are deleted.  Only the original owner of a series may delete it.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_delete_with_http_info(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_delete" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_delete`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}', 'DELETE',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_get(self, series_id, **kwargs):  # noqa: E501
        """Get all versions of a time series.  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_get(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str glob: Filters the result according to a glob pattern applied on the short name of the requested object. 
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :return: list[VersionedSeries]
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_get_with_http_info(series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_get_with_http_info(series_id, **kwargs)  # noqa: E501
            return data

    def series_series_id_get_with_http_info(self, series_id, **kwargs):  # noqa: E501
        """Get all versions of a time series.  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_get_with_http_info(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str glob: Filters the result according to a glob pattern applied on the short name of the requested object. 
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :return: list[VersionedSeries]
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'glob', 'deep', 'brief', 'audit', 'status']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_get`")  # noqa: E501

        if self.api_client.client_side_validation and ('glob' in params and
                                                       len(params['glob']) > 100):
            raise ValueError("Invalid value for parameter `glob` when calling `series_series_id_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('glob' in params and
                                                       len(params['glob']) < 1):
            raise ValueError("Invalid value for parameter `glob` when calling `series_series_id_get`, length must be greater than or equal to `1`")  # noqa: E501
        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []
        if 'glob' in params:
            query_params.append(('glob', params['glob']))  # noqa: E501
        if 'deep' in params:
            query_params.append(('deep', params['deep']))  # noqa: E501
        if 'brief' in params:
            query_params.append(('brief', params['brief']))  # noqa: E501
        if 'audit' in params:
            query_params.append(('audit', params['audit']))  # noqa: E501
        if 'status' in params:
            query_params.append(('status', params['status']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='list[VersionedSeries]',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_latest_get(self, series_id, **kwargs):  # noqa: E501
        """Get the latest version of the description of a time series  # noqa: E501

        The latest version is determined according to semantic versioning rules (e.g. v1.2.3 < v1.2.4).  Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_latest_get(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_latest_get_with_http_info(series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_latest_get_with_http_info(series_id, **kwargs)  # noqa: E501
            return data

    def series_series_id_latest_get_with_http_info(self, series_id, **kwargs):  # noqa: E501
        """Get the latest version of the description of a time series  # noqa: E501

        The latest version is determined according to semantic versioning rules (e.g. v1.2.3 < v1.2.4).  Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_latest_get_with_http_info(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :param str status: Filter the result according to a given status.  Only series in the \"PUBLISHED\" status are returned to the public.  Series owner may consult their series in any status. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'deep', 'brief', 'audit', 'status']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_latest_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_latest_get`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []
        if 'deep' in params:
            query_params.append(('deep', params['deep']))  # noqa: E501
        if 'brief' in params:
            query_params.append(('brief', params['brief']))  # noqa: E501
        if 'audit' in params:
            query_params.append(('audit', params['audit']))  # noqa: E501
        if 'status' in params:
            query_params.append(('status', params['status']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}/latest', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='VersionedSeries',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_latest_values_get(self, series_id, **kwargs):  # noqa: E501
        """Get the values from the latest version of a time series  # noqa: E501

        The latest version is determined according to semantic versioning rules (e.g. v1.2.3 < v1.2.4).  Values are returned in chronological order.  If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_latest_values_get(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_latest_values_get_with_http_info(series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_latest_values_get_with_http_info(series_id, **kwargs)  # noqa: E501
            return data

    def series_series_id_latest_values_get_with_http_info(self, series_id, **kwargs):  # noqa: E501
        """Get the values from the latest version of a time series  # noqa: E501

        The latest version is determined according to semantic versioning rules (e.g. v1.2.3 < v1.2.4).  Values are returned in chronological order.  If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_latest_values_get_with_http_info(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', '_from', 'to', 'convert']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_latest_values_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_latest_values_get`")  # noqa: E501

        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) > 100):
            raise ValueError("Invalid value for parameter `convert` when calling `series_series_id_latest_values_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) < 1):
            raise ValueError("Invalid value for parameter `convert` when calling `series_series_id_latest_values_get`, length must be greater than or equal to `1`")  # noqa: E501
        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []
        if '_from' in params:
            query_params.append(('from', params['_from']))  # noqa: E501
        if 'to' in params:
            query_params.append(('to', params['to']))  # noqa: E501
        if 'convert' in params:
            query_params.append(('convert', params['convert']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json', 'text/csv'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}/latest/values', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='TsValues',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_post(self, series_id, series, **kwargs):  # noqa: E501
        """Creates a new version of a time series  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_post(series_id, series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_post_with_http_info(series_id, series, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_post_with_http_info(series_id, series, **kwargs)  # noqa: E501
            return data

    def series_series_id_post_with_http_info(self, series_id, series, **kwargs):  # noqa: E501
        """Creates a new version of a time series  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_post_with_http_info(series_id, series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'series']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_post`")  # noqa: E501
        # verify the required parameter 'series' is set
        if self.api_client.client_side_validation and ('series' not in params or
                                                       params['series'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series` when calling `series_series_id_post`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'series' in params:
            body_params = params['series']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_put(self, series_id, series, **kwargs):  # noqa: E501
        """Updates metadata about a time series  # noqa: E501

        This action only updates metadata about the series (such as associated themes, tags, etc) and does not create a new version.  Only owners registered for a series may update the series.  Any time series values specified in the input are ignored. One must update a specific version to modify the values of a time series.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_put(series_id, series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_put_with_http_info(series_id, series, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_put_with_http_info(series_id, series, **kwargs)  # noqa: E501
            return data

    def series_series_id_put_with_http_info(self, series_id, series, **kwargs):  # noqa: E501
        """Updates metadata about a time series  # noqa: E501

        This action only updates metadata about the series (such as associated themes, tags, etc) and does not create a new version.  Only owners registered for a series may update the series.  Any time series values specified in the input are ignored. One must update a specific version to modify the values of a time series.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_put_with_http_info(series_id, series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param Series series: The description of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'series']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_put" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_put`")  # noqa: E501
        # verify the required parameter 'series' is set
        if self.api_client.client_side_validation and ('series' not in params or
                                                       params['series'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series` when calling `series_series_id_put`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'series' in params:
            body_params = params['series']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}', 'PUT',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_semver_get(self, series_id, **kwargs):  # noqa: E501
        """Get all semver tags associated to a series  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_get(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :return: list[str]
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_semver_get_with_http_info(series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_semver_get_with_http_info(series_id, **kwargs)  # noqa: E501
            return data

    def series_series_id_semver_get_with_http_info(self, series_id, **kwargs):  # noqa: E501
        """Get all semver tags associated to a series  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_get_with_http_info(series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :return: list[str]
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_semver_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_semver_get`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}/semver', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='list[str]',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_semver_semver_get(self, series_id, semver, **kwargs):  # noqa: E501
        """Get a version of a time series with a semver tag  # noqa: E501

        Semver tags follow semantic versioning rules (e.g. v1.2.3, v1.2.3-rc1, v1.3.0 ...).  Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_semver_get(series_id, semver, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str semver: The semver tag of a series' version. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_semver_semver_get_with_http_info(series_id, semver, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_semver_semver_get_with_http_info(series_id, semver, **kwargs)  # noqa: E501
            return data

    def series_series_id_semver_semver_get_with_http_info(self, series_id, semver, **kwargs):  # noqa: E501
        """Get a version of a time series with a semver tag  # noqa: E501

        Semver tags follow semantic versioning rules (e.g. v1.2.3, v1.2.3-rc1, v1.3.0 ...).  Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_semver_get_with_http_info(series_id, semver, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str semver: The semver tag of a series' version. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'semver', 'deep', 'brief', 'audit']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_semver_semver_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_semver_semver_get`")  # noqa: E501
        # verify the required parameter 'semver' is set
        if self.api_client.client_side_validation and ('semver' not in params or
                                                       params['semver'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `semver` when calling `series_series_id_semver_semver_get`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501
        if 'semver' in params:
            path_params['semver'] = params['semver']  # noqa: E501

        query_params = []
        if 'deep' in params:
            query_params.append(('deep', params['deep']))  # noqa: E501
        if 'brief' in params:
            query_params.append(('brief', params['brief']))  # noqa: E501
        if 'audit' in params:
            query_params.append(('audit', params['audit']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}/semver/{semver}', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='VersionedSeries',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_series_id_semver_semver_values_get(self, series_id, semver, **kwargs):  # noqa: E501
        """Get the values of version of a time series with a semver tag  # noqa: E501

         If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_semver_values_get(series_id, semver, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str semver: The semver tag of a series' version. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_series_id_semver_semver_values_get_with_http_info(series_id, semver, **kwargs)  # noqa: E501
        else:
            (data) = self.series_series_id_semver_semver_values_get_with_http_info(series_id, semver, **kwargs)  # noqa: E501
            return data

    def series_series_id_semver_semver_values_get_with_http_info(self, series_id, semver, **kwargs):  # noqa: E501
        """Get the values of version of a time series with a semver tag  # noqa: E501

         If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_series_id_semver_semver_values_get_with_http_info(series_id, semver, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param int series_id: The unique ID of a time series. (required)
        :param str semver: The semver tag of a series' version. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['series_id', 'semver', '_from', 'to', 'convert']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_series_id_semver_semver_values_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'series_id' is set
        if self.api_client.client_side_validation and ('series_id' not in params or
                                                       params['series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `series_id` when calling `series_series_id_semver_semver_values_get`")  # noqa: E501
        # verify the required parameter 'semver' is set
        if self.api_client.client_side_validation and ('semver' not in params or
                                                       params['semver'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `semver` when calling `series_series_id_semver_semver_values_get`")  # noqa: E501

        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) > 100):
            raise ValueError("Invalid value for parameter `convert` when calling `series_series_id_semver_semver_values_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) < 1):
            raise ValueError("Invalid value for parameter `convert` when calling `series_series_id_semver_semver_values_get`, length must be greater than or equal to `1`")  # noqa: E501
        collection_formats = {}

        path_params = {}
        if 'series_id' in params:
            path_params['seriesId'] = params['series_id']  # noqa: E501
        if 'semver' in params:
            path_params['semver'] = params['semver']  # noqa: E501

        query_params = []
        if '_from' in params:
            query_params.append(('from', params['_from']))  # noqa: E501
        if 'to' in params:
            query_params.append(('to', params['to']))  # noqa: E501
        if 'convert' in params:
            query_params.append(('convert', params['convert']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json', 'text/csv'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/{seriesId}/semver/{semver}/values', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='TsValues',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_delete(self, versioned_series_id, **kwargs):  # noqa: E501
        """Deletes a version of a time series  # noqa: E501

        Only the original owner of this version of the series may delete it.  All values associated to that version are deleted.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_delete(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_delete_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_delete_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_delete_with_http_info(self, versioned_series_id, **kwargs):  # noqa: E501
        """Deletes a version of a time series  # noqa: E501

        Only the original owner of this version of the series may delete it.  All values associated to that version are deleted.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_delete_with_http_info(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_delete" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_delete`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}', 'DELETE',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_get(self, versioned_series_id, **kwargs):  # noqa: E501
        """Get a version of a time series  # noqa: E501

        Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_get(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_get_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_get_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_get_with_http_info(self, versioned_series_id, **kwargs):  # noqa: E501
        """Get a version of a time series  # noqa: E501

        Unless requested by the query parameters, the time series values are not returned by default.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_get_with_http_info(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param bool deep: When deep is specified, the response will contain a deep representation of the object, rather than just a shallow description. 
        :param bool brief: When brief is specified, the response will only contain essential data and strip long descriptions. 
        :param bool audit: When audit is specified, the response will also contain the audit trail field. 
        :return: VersionedSeries
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', 'deep', 'brief', 'audit']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_get`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []
        if 'deep' in params:
            query_params.append(('deep', params['deep']))  # noqa: E501
        if 'brief' in params:
            query_params.append(('brief', params['brief']))  # noqa: E501
        if 'audit' in params:
            query_params.append(('audit', params['audit']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='VersionedSeries',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_post(self, versioned_series_id, version_series, **kwargs):  # noqa: E501
        """creates a new version of a time series  # noqa: E501

        A conflict is reported if the version was already attributed to a version of the same series. Use PUT to update existing values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_post(versioned_series_id, version_series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param VersionedSeries version_series: The description of a version of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_post_with_http_info(versioned_series_id, version_series, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_post_with_http_info(versioned_series_id, version_series, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_post_with_http_info(self, versioned_series_id, version_series, **kwargs):  # noqa: E501
        """creates a new version of a time series  # noqa: E501

        A conflict is reported if the version was already attributed to a version of the same series. Use PUT to update existing values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_post_with_http_info(versioned_series_id, version_series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param VersionedSeries version_series: The description of a version of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', 'version_series']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_post`")  # noqa: E501
        # verify the required parameter 'version_series' is set
        if self.api_client.client_side_validation and ('version_series' not in params or
                                                       params['version_series'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `version_series` when calling `series_versions_versioned_series_id_post`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'version_series' in params:
            body_params = params['version_series']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_put(self, versioned_series_id, version_series, **kwargs):  # noqa: E501
        """Updates the metadata of version of a time series  # noqa: E501

        This endpoint replaces the metadata for the requested version, without creating a new version.  Only the owners of the time series may modify values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_put(versioned_series_id, version_series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param VersionedSeries version_series: The description of a version of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_put_with_http_info(versioned_series_id, version_series, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_put_with_http_info(versioned_series_id, version_series, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_put_with_http_info(self, versioned_series_id, version_series, **kwargs):  # noqa: E501
        """Updates the metadata of version of a time series  # noqa: E501

        This endpoint replaces the metadata for the requested version, without creating a new version.  Only the owners of the time series may modify values.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_put_with_http_info(versioned_series_id, version_series, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param VersionedSeries version_series: The description of a version of a time series.  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', 'version_series']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_put" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_put`")  # noqa: E501
        # verify the required parameter 'version_series' is set
        if self.api_client.client_side_validation and ('version_series' not in params or
                                                       params['version_series'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `version_series` when calling `series_versions_versioned_series_id_put`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'version_series' in params:
            body_params = params['version_series']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}', 'PUT',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_values_delete(self, versioned_series_id, **kwargs):  # noqa: E501
        """Deletes the values attached to a version of a time series  # noqa: E501

        Only the original owner of this version of the series may delete it.  The version is not deleted. Only the values associated to that version are deleted.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_delete(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_values_delete_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_values_delete_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_values_delete_with_http_info(self, versioned_series_id, **kwargs):  # noqa: E501
        """Deletes the values attached to a version of a time series  # noqa: E501

        Only the original owner of this version of the series may delete it.  The version is not deleted. Only the values associated to that version are deleted.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_delete_with_http_info(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_values_delete" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_values_delete`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}/values', 'DELETE',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_values_get(self, versioned_series_id, **kwargs):  # noqa: E501
        """Get the values of version of a time series  # noqa: E501

        If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_get(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_values_get_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_values_get_with_http_info(versioned_series_id, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_values_get_with_http_info(self, versioned_series_id, **kwargs):  # noqa: E501
        """Get the values of version of a time series  # noqa: E501

        If the request negotiates a response MIME type with text/csv (with the Accept header), this endpoint produces a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_get_with_http_info(versioned_series_id, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param datetime _from: Truncates values starting from this date (boundary is included). 
        :param datetime to: Truncates values up to this date (boundary is included). 
        :param str convert: Converts values in some other compatible measurement unit, possibly specified as a composite measurement unit. 
        :return: TsValues
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', '_from', 'to', 'convert']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_values_get" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_values_get`")  # noqa: E501

        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) > 100):
            raise ValueError("Invalid value for parameter `convert` when calling `series_versions_versioned_series_id_values_get`, length must be less than or equal to `100`")  # noqa: E501
        if self.api_client.client_side_validation and ('convert' in params and
                                                       len(params['convert']) < 1):
            raise ValueError("Invalid value for parameter `convert` when calling `series_versions_versioned_series_id_values_get`, length must be greater than or equal to `1`")  # noqa: E501
        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []
        if '_from' in params:
            query_params.append(('from', params['_from']))  # noqa: E501
        if 'to' in params:
            query_params.append(('to', params['to']))  # noqa: E501
        if 'convert' in params:
            query_params.append(('convert', params['convert']))  # noqa: E501

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json', 'text/csv'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}/values', 'GET',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='TsValues',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_values_post(self, versioned_series_id, values, **kwargs):  # noqa: E501
        """Uploads values to the version of a time series  # noqa: E501

        This endpoint creates time series values for the latest version.  Only the owners of the time series may add values.  A conflict is reported if values were already attributed to this version. Use PUT to update existing values.  If the request negotiates a request MIME type with text/csv (with the Content-Type header), this endpoint consumes a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_post(versioned_series_id, values, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param TsValues values: The values of a time series  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_values_post_with_http_info(versioned_series_id, values, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_values_post_with_http_info(versioned_series_id, values, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_values_post_with_http_info(self, versioned_series_id, values, **kwargs):  # noqa: E501
        """Uploads values to the version of a time series  # noqa: E501

        This endpoint creates time series values for the latest version.  Only the owners of the time series may add values.  A conflict is reported if values were already attributed to this version. Use PUT to update existing values.  If the request negotiates a request MIME type with text/csv (with the Content-Type header), this endpoint consumes a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_post_with_http_info(versioned_series_id, values, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param TsValues values: The values of a time series  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', 'values']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_values_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_values_post`")  # noqa: E501
        # verify the required parameter 'values' is set
        if self.api_client.client_side_validation and ('values' not in params or
                                                       params['values'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `values` when calling `series_versions_versioned_series_id_values_post`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'values' in params:
            body_params = params['values']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json', 'text/csv'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}/values', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)

    def series_versions_versioned_series_id_values_put(self, versioned_series_id, values, **kwargs):  # noqa: E501
        """Replaces the values of version of a time series  # noqa: E501

        This endpoint replaces time series values for the requested version, without creating a new version.  Only the owners of the time series may modify values.  A conflict is reported if values were already attributed to this version. Use PUT to update existing values.  If the request negotiates a request MIME type with text/csv (with the Content-Type header), this endpoint consumes a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_put(versioned_series_id, values, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param TsValues values: The values of a time series  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.series_versions_versioned_series_id_values_put_with_http_info(versioned_series_id, values, **kwargs)  # noqa: E501
        else:
            (data) = self.series_versions_versioned_series_id_values_put_with_http_info(versioned_series_id, values, **kwargs)  # noqa: E501
            return data

    def series_versions_versioned_series_id_values_put_with_http_info(self, versioned_series_id, values, **kwargs):  # noqa: E501
        """Replaces the values of version of a time series  # noqa: E501

        This endpoint replaces time series values for the requested version, without creating a new version.  Only the owners of the time series may modify values.  A conflict is reported if values were already attributed to this version. Use PUT to update existing values.  If the request negotiates a request MIME type with text/csv (with the Content-Type header), this endpoint consumes a CSV file.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.series_versions_versioned_series_id_values_put_with_http_info(versioned_series_id, values, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str versioned_series_id: The UUID of a version of the time series. (required)
        :param TsValues values: The values of a time series  (required)
        :return: None
                 If the method is called asynchronously,
                 returns the request thread.
        """

        all_params = ['versioned_series_id', 'values']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')

        params = locals()
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method series_versions_versioned_series_id_values_put" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'versioned_series_id' is set
        if self.api_client.client_side_validation and ('versioned_series_id' not in params or
                                                       params['versioned_series_id'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `versioned_series_id` when calling `series_versions_versioned_series_id_values_put`")  # noqa: E501
        # verify the required parameter 'values' is set
        if self.api_client.client_side_validation and ('values' not in params or
                                                       params['values'] is None):  # noqa: E501
            raise ValueError("Missing the required parameter `values` when calling `series_versions_versioned_series_id_values_put`")  # noqa: E501

        collection_formats = {}

        path_params = {}
        if 'versioned_series_id' in params:
            path_params['versionedSeriesId'] = params['versioned_series_id']  # noqa: E501

        query_params = []

        header_params = {}

        form_params = []
        local_var_files = {}

        body_params = None
        if 'values' in params:
            body_params = params['values']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501

        # HTTP header `Content-Type`
        header_params['Content-Type'] = self.api_client.select_header_content_type(  # noqa: E501
            ['application/json', 'text/csv'])  # noqa: E501

        # Authentication setting
        auth_settings = []  # noqa: E501

        return self.api_client.call_api(
            '/series/versions/{versionedSeriesId}/values', 'PUT',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type=None,  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
            _request_timeout=params.get('_request_timeout'),
            collection_formats=collection_formats)
